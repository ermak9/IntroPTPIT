Game.LevelCheck = function(game){
    game = this.game;
};

Game.LevelCheck.prototype = {create: createLevelCheck, update: update, shutdown: shutdown};
...
function createTextMass(){
  console.log("levelCheck: createTExtMass:")
  for (var i = 0; i < objects.length; i++){
...
function listenerButtonCheck(){
  if ( isObjectDelivered){
    ...
    if ( forceRight === forceLeft){
      text.text = "Молодец! У тебя получилось!";
      soundWin.play();
      buttonNext.visible = true;
    } else {
      buttonTryAgain.visible = true;
      text.text = "Попробуй снова";
      soundLose.play();
    }
    bodies = [];
  }
